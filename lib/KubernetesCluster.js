...
const POD_LIMIT_TIME = process.env.POD_LIMIT_TIME || 10 * 60 * 1000 // Time in ms
...
class KubernetesCluster extends Cluster {
  ...
  async spawn (environId, pool, reason) {
    environId = environId || Object.keys(this.containers)[0]

    const time = new Date().toISOString().replace(/[-T:.Z]/g, '')
    const rand = Moniker.choose()
