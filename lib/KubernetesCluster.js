...
class KubernetesCluster extends Cluster {
  ...
  async stop (podId) {
    this._pods.delete({ name: podId, gracePeriodSeconds: POD_GRACE_PERIOD })
  ...
  async clean () {
    const response = await this._pods.get({
      qs: { labelSelector: 'pool!=deleting' }
    })
    const pods = response.body
    ...
    for (let pod of pods.items) {
      if (['Succeeded', 'Failed'].indexOf(pod.status.phase) > -1) {
        this._pods.delete({ name: pod.metadata.name })
        ...
        this._pods.delete({ name: patchedPod.metadata.name, gracePeriodSeconds: POD_GRACE_PERIOD })
