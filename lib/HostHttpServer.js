...
async function sessionCreate (ctx) {
  ...
  // Create a session
  const sessionId = await ctx._host.sessionCreate(ctx._user, environId)
  if (ctx.request.query.stream !== undefined) {
    ...
    const update = setInterval(async () => {
      let session = await ctx._host.sessionGet(ctx._user, sessionId)
      stream.write('event: update\ndata: ' + JSON.stringify(session) + '\n\n')
      // End stream when session is running and has an IP address
      if (session.phase === 'running' && session.ip) finish()
    }, SSE_INTERVAL)
    // Stop update when request is closed
    const finish = () => {
