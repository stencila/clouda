...
class HostHttpServer {
  ...
  start () {
    ...
    app.route('GET', '/v1/manifest', (req, res, ctx) => {
    ...
    app.route('POST', '/v1/environ/*', (req, res, ctx) => {
      receive(req, res, ctx, /\/v1\/environ\/(.+)/, (match, session, body) => {
    ...
    app.route('GET', '/v1/environ/*', (req, res, ctx) => {
      receive(req, res, ctx, /\/v1\/environ\/(.+)/, (match, session, body) => {
        if (!session) return error(req, res, ctx, 401, 'Authentication required')
    ...
    app.route(['GET', 'POST', 'PUT', 'DELETE'], '/v1/proxy/*', (req, res, ctx) => {
      receive(req, res, ctx, /\/v1\/proxy\/([^/]+)(.*)/, (match, session, body) => {
        if (!session) return error(req, res, ctx, 401, 'Authentication required')
