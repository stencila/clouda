...
class Host {
  constructor () {
    ...
    this.environs = [
      {
        id: 'stencila/core@sha256:199395144df74670360b39dc323830aaa22856bbd244ca694f7eb030a416ed3f',
        name: 'stencila/core',
        version: 'stencila/core@sha256:199395144df74670360b39dc323830aaa22856bbd244ca694f7eb030a416ed3f'
      }
    ]
   ...
   * Create a session
   *
   * Currently, if the user has an existing session then
   * this will return that session, regardless of the
  ...
  async sessionCreate (user, environId) {
    ...
    if (user.sessionId) {
      ...
      sessionId = await this.cluster.acquire(environId)
